...
namespace EDIVE.NativeUtils.TeamCity
{
    ...
    public static class TeamCityServiceMessages
    {
        ...
        public static void BuildStop(string comment, bool reAddToQueue = false)
        {
        ...
        /// <summary>
        /// For setting the final build status that persists after a build finishes.
        /// </summary>
        /// <param name="text">Text of the status </param>
        public static void SetBuildStatus(string text)
        {
            Console.WriteLine($"##teamcity[buildStatus text='{ReformatText(text)}']");
        }

        /// <summary>
        /// For setting the final build status that persists after a build finishes.
        /// Changes the visual build status in the TeamCity UI.
        /// It is recommended to use MessageBuildProblem to report problems instead of changing the build status to failed.
        /// </summary>
        /// <param name="text">Text of the status</param>
        /// <param name="status">New visual build status</param>
        public static void SetBuildStatus(string text, BuildStatusType status)
        {
            Console.WriteLine($"##teamcity[buildStatus status='{status.ToString().ToUpper()}' text='{ReformatText(text)}']");
        }
        
        /// <summary>
        /// To set a custom build number directly.
        /// you can use the {build.number} substitution to use the current build number automatically generated by TeamCity. For example: "1.2.3_{build.number}-ent"
        /// </summary>
        ...
        /// <summary>
        /// Allows you to add new build tags.
        /// </summary>
        ...
        /// <summary>
        /// Allows you to remove existing build tags.
        /// </summary>
        ...
        /// <summary>
        /// Allows you to report statistical data to teamcity to further use it in reports and charts.
        /// </summary>
        /// <param name="key">Key of the statistics, must not be equal to any of predefined keys</param>
        /// <param name="value">value of the statistics</param>
        public static void ReportStatistics(string key, int value)
        {
            Console.WriteLine($"##teamcity[buildStatisticValue key='{ReformatText(key)}' value='{value.ToString()}']");
        }
        ...
        /// <param name="key">Key of the statistics, must not be equal to any of predefined keys</param>
        /// <param name="value">value of the statistics</param>
        public static void ReportStatistics(string key, float value)
        {
            Console.WriteLine($"##teamcity[buildStatisticValue key='{ReformatText(key)}' value='{value.ToString(CultureInfo.InvariantCulture)}']");
        }
        
        /// <summary>
        /// To publish the build artifacts while the build is still running, immediately after the artifacts are built.
        /// Call after all the files are ready and no file is locked for reading.
        /// Artifacts are uploaded in the background, which can take time. Make sure the matching files are not deleted till the end of the build step.
        /// </summary>
        /// <param name="path">Path to the artifact</param>
        public static void PublishArtifacts(string path)
        {
            Console.WriteLine($"##teamcity[publishArtifacts '{ReformatText(path)}']");
        }
        ...
        public static void SetParameter(string name, string value, ParameterType parameterType = ParameterType.ConfigurationParameter)
        {
