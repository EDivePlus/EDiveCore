...
Shader "EDIVE/Skybox/Cubemap Blend"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 _BlendTex_HDR;   // HDR decode parameters for second cubemap
            ...
            inline half3 DecodeHDR_CubeA(float4 data)
            {
                return DecodeHDR(data, _MainTex_HDR);
            }
            inline half3 DecodeHDR_CubeB(float4 data)
            {
                return DecodeHDR(data, _BlendTex_HDR);
            }
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // Sample and decode both HDR cubemaps
                half4 s0 = texCUBE(_MainTex,      i.dirWS);
                half4 s1 = texCUBE(_BlendTex, i.dirWS);
                half3 c0 = DecodeHDR_CubeA(s0);
                half3 c1 = DecodeHDR_CubeB(s1);

                // Blend, then tint/expose and handle color space
                half3 blendedRGB = lerp(c0, c1, _BlendFactor);
                half4 skyRGBA    = float4(blendedRGB, 0.0) * unity_ColorSpaceDouble * _Tint * _Exposure;
